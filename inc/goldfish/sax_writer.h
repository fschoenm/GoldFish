...
namespace goldfish { namespace sax
{
	...
	public:
		...
		void write_key(const uint64_t t) { append_key().write(t); }
		...
		template <class U> void write(const uint64_t key, U&& value)
		{
			write_key(key);
			write_value(std::forward<U>(value));
		}
	...
	public:
		...
		auto write(const std::string_view& text)
		{
			...
			stream.write_buffer({ reinterpret_cast<const byte*>(text.data()), narrow_cast<std::ptrdiff_t>(text.size()) });
			return stream.flush();
		}
	...
	private:
		...
		auto copy(Stream& s, CreateWriterWithSize&& create_writer_with_size, CreateWriterWithoutSize&& create_writer_without_size)
		{
			...
			if (cb < sizeof(buffer))
			{
				...
				output_stream.write_buffer({ buffer, narrow_cast<std::ptrdiff_t>(cb) });
